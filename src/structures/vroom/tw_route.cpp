  ...
  while (current_job != last_job || current_break != last_break) {
    ...
    if (current_break == last_break) {     
      ...
      if (check_max_load) {
       ...
       // -- avoid return to depot with undelivered jobs START ------------------------
      // BRUNO: check that the insertion does not force a return to depot with undelivered jobs

      if (false){//j.location_index()==0){
        for (size_t r = first_rank; r < route.size(); r++) {
          ...
          if (j.type == JOB_TYPE::DELIVERY) {
            ...
            assert(s >= 0); // we should have found the matching pickup
          }

          TTRACE_LOG(" route[" << r << "]:" << route[r]
                              << " earliest:" << t2str(earliest[r])
                              << " latest:" << t2str(latest[r])
                              << " action_time:" << t2str(action_time[r]));
        }
      }
      // -- avoid return to depot with undelivered jobs END ------------------------
