...
/**
 BRUNO: this predicates check that the insertion of first_rank_location_index at first_rank will not cause a return 
 to depot with undelivered jobs amongst jobs in existing route.

 this is a quick implementation, not optimized at all

 
 param first_rank: the rank where we are inserting a job
 param first_rank_location_index: the location index of the job being inserted at first_rank
 if first_rank_location_index==-1 it means to check the route as it is, without considering any insertion

 we need to handle:
 - pickup at depot delivery at customer 
 - pickup at customer delivery at depot 
 - pickup at customer delivery at another customer 

 the idea is to count the number of times we enter or leave the depot between each pickup and delivery.
 it should be either 0 (pickup and delivery both at customers) 
 or 1 (pickup at depot and delivery at customer or viceversa)
