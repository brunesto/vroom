...
RouteInsertion compute_best_insertion(const Input& input,
  ...
  if (insert.eval != NO_EVAL) {
    ...
    // LLM: Since PD jobs involve two stops (pickup and delivery), we divide the cost by 2
    // LLM: to make it comparable to single jobs when evaluating insertion cost.
    insert.eval.cost =
...
void LocalSearch<Route,
  ...
  // at first).
  // LLM: Initialize the list of vehicle pairs (source, target) to be considered for local search moves.
  // LLM: Only pairs where the source vehicle is compatible with the target vehicle are added.
  ...
  while (best_gain.cost > 0 || best_priority > 0) {    
          ...
          for (unsigned s_rank = 0; s_rank < _sol[source].size(); ++s_rank) {
            ...
            if (best_priorities[source] <= priority_gain) {
              ...
              for (unsigned t_rank = begin_t_rank; t_rank <= end_t_rank;
                ...
                if (better_if_valid && r.is_valid2()) {
                  best_priorities[source] = priority_gain;
    ...
    for (const auto& [source, target] : s_t_pairs) {
      ...
      for (unsigned s_rank = 0; s_rank < _sol[source].size() - 1; ++s_rank) {
        ...
        for (unsigned t_rank = begin_t_rank; t_rank < end_t_rank; ++t_rank) {
          ...
          if (current_best < r.gain_upper_bound() && r.is_valid2() &&
              current_best < r.gain()) {
            current_best = r.gain();
    ...
    for (const auto& [source, target] : s_t_pairs) {
      ...
      for (unsigned s_rank = first_s_rank; s_rank < _sol[source].size();
        ...
        for (int t_rank = end_t_rank - 1; t_rank >= first_t_rank; --t_rank) {
          ...
          if (best_gains[source][target] < r.gain() && r.is_valid2()) {
    ...
    if (_input.apply_TSPFix() && !_input.has_shipments()) {
      for (const auto& [source, target] : s_t_pairs) {
        ...
        if (best_gains[source][target] < op.gain() && op.is_valid2()) {
    ...
    for (const auto& [source, target] : s_t_pairs) {
      ...
      for (unsigned s_rank = 0; s_rank < _sol[source].size() - 2; ++s_rank) {
        ...
        for (Index t_rank = s_rank + 2; t_rank < end_t_rank; ++t_rank) {
          ...
          if (best_gains[source][source] < r.gain() && r.is_valid2()) {
            best_gains[source][source] = r.gain();
      ...
      for (unsigned s_rank = 0; s_rank < _sol[source].size() - 2; ++s_rank) {
        ...
        for (unsigned t_rank = s_rank + 2; t_rank < end_t_rank; ++t_rank) {
          IntraTwoOpt r(_input,
                        ...
                        t_rank);
          auto& current_best = best_gains[source][target];
          if (current_best < r.gain() && r.is_valid2()) {
            current_best = r.gain();
    ...
    if (_input.has_shipments()) {
      ...
      for (const auto& [source, target] : s_t_pairs) {
        ...
        for (unsigned s_p_rank = 0; s_p_rank < _sol[source].size();
          ...
          if (best_gains[source][target] < pdr.gain() && pdr.is_valid2()) {
    ...
    if (!_input.has_homogeneous_locations() ||
      ...
      for (const auto& [source, target] : s_t_pairs) {
        ...
        if (best_gains[source][target] < re.gain() && re.is_valid2()) {
    ...
    // LLM: Priority improvements take precedence over cost improvements.
    best_priority = 0;
    ...
    if (best_priority > 0 || best_gain.cost > 0) {
     ...
     // assert(!is_return_to_depot_with_undelivered_jobs_no_insertion(_input,&_sol[ best_ops[best_source][best_target]->t_rank].route));
      ...
      // LLM: Reset best gains and operators for routes that were modified.
      // LLM: Re-initialize the list of vehicle pairs to check in the next iteration,
      // LLM: focusing on pairs involving the modified routes.
      s_t_pairs.clear();
      for (auto v_rank : update_candidates) {
